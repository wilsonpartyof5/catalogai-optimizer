import {
  db
} from "/build/_shared/chunk-7LEGIGN6.js";
import {
  createHotContext
...
if (import.meta) {
  ...
  import.meta.hot.lastModified = "1760367606093.0757";
}
...
if (import.meta) {
  ...
  import.meta.hot.lastModified = "1760464569780.4194";
}
var AIEnrichmentService = class {
  ...
  async generateRecommendationForGap(gap, baseSpec, userId) {
    ...
    const mapping = fieldMappings[gap];
    if (!mapping) {
    ...
    try {
      ...
      console.log(`\u{1F916} AI Response for ${gap}:`, aiResponse);
      if (aiResponse && aiResponse.trim() !== "" && !aiResponse.toLowerCase().includes("error") && aiResponse.trim().toLowerCase() !== "n/a") {
        return {
          field: gap,
          originalValue: baseSpec[gap] || null,
          newValue: aiResponse,
  ...
  async enrichProducts(userId, products, options = {}) {
    ...
    for (const product of products) {
      try {
        const { mapShopifyToSpec, calculateProductScore } = await import("/build/_shared/fieldMapper-CB5YLAG6.js");
  ...
  async applyEnrichmentToShopify(userId, shopDomain, accessToken, enrichmentResult) {
    try {
      ...
      for (const improvement of enrichmentResult.improvements) {
        ...
        console.log(`\u{1F4DD} Applying ${field}: ${newValue}`);
...
//# sourceMappingURL=/build/_shared/aiEnrich-COJARUHM.js.map
